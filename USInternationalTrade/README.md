...
following two aspects of ATSD to look into this dataset: interactive graphs from [Chart Lab](/ChartLabIntro/README.md) and tabular outputs from analytical SQL queries with support for [partitioning](https://github.com/axibase/atsd/blob/master/docs/api/sql/README.md#partitioning).
...
ATSD handles this by implementing a [schema-based](https://github.com/axibase/atsd/blob/master/docs/parsers/csv/csv-schema.md) parser which can be configured to load records from non-standard CSV files, such as the BLS report.
...
In addition to looking at graphical outputs, we can also perform [SQL queries](https://github.com/axibase/atsd/blob/master/docs/api/sql/README.md#overview), which can be used
...
2. [Install the ATSD database](https://github.com/axibase/atsd/blob/master/docs/installation/docker.md#option-2-configure-collector-account-manually) on in your local Docker configuration.
